...
interface StoreDetailProps {
  branch: "plaza" | "palladium" | "khalifa";
  onBack: () => void;
}
...
export default function StoreDetail({ branch, onBack }: StoreDetailProps) {
  ...
  const themeColors = {
    plaza: "from-[#05120a] via-black to-black",
    palladium: "from-[#1a140b] via-black to-black",
    khalifa: "from-[#141416] via-black to-black",
  };
  ...
  const logoSrc = {
    plaza: "/logo_plaza.png",
    palladium: "/logo_palladium.png",
    khalifa: "/logo_khalifa.png",
  };

  const name = {
    plaza: "Rahman Plaza",
    palladium: "Rahman Palladium",
    khalifa: "Rahman Khalifa",
  };
  ...
  return (
    <div className={`min-h-screen bg-black text-warm-white overflow-x-hidden relative`}>
      {/* Background Ambient Lighting & Glow */}
      <div className={`fixed inset-0 bg-gradient-to-br ${themeColors[branch]} opacity-60 pointer-events-none z-0`} />
      
      {/* Warm Orange Glow Reflective Orb */}
      <div className="absolute top-[20%] right-[10%] w-[50vw] h-[50vw] bg-[#D4AF37]/5 rounded-full blur-[150px] pointer-events-none z-0" />
      <div className="absolute bottom-[10%] left-[5%] w-[40vw] h-[40vw] bg-[#0A2E1C]/10 rounded-full blur-[120px] pointer-events-none z-0" />
      ...
      {/* Header */}
      <header className="relative z-50 p-4 md:p-8 flex justify-between items-center bg-black/50 backdrop-blur-md border-b border-white/5">
        <button 
          onClick={onBack}
          className="flex items-center gap-2 text-warm-white/60 hover:text-gold transition-colors group cursor-pointer"
        >
        ...
        <div className="flex flex-col items-center">
            <img src={logoSrc[branch]} alt={name[branch]} className="h-10 md:h-12 w-auto mb-1 md:mb-2" />
            <span className={`text-[9px] md:text-xs uppercase tracking-[0.2em] md:tracking-[0.4em] font-medium ${accentColor[branch]} text-center px-2`}>
                {branch === 'plaza' ? 'The flagship store' : branch === 'palladium' ? 'Fine Jewelry' : 'Premium Shoes'}
            </span>
        </div>

        <div className="w-12 md:w-24 hidden md:block" />
      </header>

      {/* Main Luxury Layout */}
      <main className="relative z-10 max-w-7xl mx-auto py-20 px-6 md:px-12 flex flex-col lg:flex-row gap-16 lg:gap-24 min-h-[750px] items-center lg:items-stretch">
        ...
        <div className="w-full lg:w-[35%] flex flex-col justify-center gap-12 relative z-20 self-center lg:self-auto">
          <div className="space-y-4">
            ...
            <h1 className="text-4xl md:text-6xl font-display font-light uppercase tracking-tighter leading-none">
              Explore <br />
            ...
            <p className="text-warm-white/40 text-sm max-w-sm font-light leading-relaxed">
              Discover the curated layout of {name[branch]}. Every floor is designed for an unparalleled experience.
            </p>
          </div>
        ...
        <div className="w-full lg:w-[65%] flex flex-col justify-center min-h-[550px] relative z-10">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="relative w-full aspect-[16/10] md:aspect-[16/9] rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.5)] border border-white/15 bg-white/[0.02] p-2 group tilt-card">
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 bg-black/85 backdrop-blur-md">
                ...
                >
                    ...
                    <div className="mt-12 p-6 rounded-2xl bg-white/[0.02] border border-white/5 text-sm text-warm-white/40 leading-relaxed italic">
                        "Designed with precision and luxury in mind, {floorData[showItems].name} represents the core values of {name[branch]} - Excellence in architecture and retail."
                    </div>
                </motion.div>
            </div>
        )}
